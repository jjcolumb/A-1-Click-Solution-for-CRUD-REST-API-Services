...
using MySolutionAPI.WebApi.JWT;
using DevExpress.ExpressApp.Security.Authentication;
using DevExpress.ExpressApp.Security.Authentication.ClientServer;
using MySolutionAPI.WebApi.Core;
using DevExpress.ExpressApp.AspNetCore.WebApi;

namespace MySolutionAPI.WebApi;
...
public class Startup {
    ...
    public void ConfigureServices(IServiceCollection services) {
        ...
        services.AddSwaggerGen(c => {
            ...
            c.SwaggerDoc("v1", new OpenApiInfo {
                Title = "MySolutionAPI API",
                Version = "v1",
                Description = @"Use AddXafWebApi(options) in the MySolutionAPI.WebApi\Startup.cs file to make Business Objects available in the Web API."
            });
                ...
                Flows = new OpenApiOAuthFlows() {
                    AuthorizationCode = new OpenApiOAuthFlow() {
                        ...
                        Scopes = new Dictionary<string, string> {
                            ...
                            { @"[Enter the scope name in the MySolutionAPI.WebApi\Startup.cs file]", @"[Enter the scope description in the MySolutionAPI.WebApi\Startup.cs file]" }
                        }
                    }
                }
            });
    ...
    public void Configure(IApplicationBuilder app, IWebHostEnvironment env) {
        if(env.IsDevelopment()) {
            ...
            app.UseSwagger();
            app.UseSwaggerUI(c => {
                c.SwaggerEndpoint("/swagger/v1/swagger.json", "MySolutionAPI WebApi v1");
